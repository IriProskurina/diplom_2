...
class TestLoginUser:
    ...
    def test_login_existing_user_success(self, api_client):
        ...
        # Исключаем поле 'created_user' перед регистрацией
        filtered_data = {k: v for k, v in data.items() if k != 'created_user'}
        client.register_user(**filtered_data)
        data['created_user'] = True  # Восстанавливаем признак создания пользователя
    ...
    def test_login_with_numeric_password(self, api_client):
        ...
        response = client.login_user(data['email'], numeric_password)
        assert response.status_code == 401, f"Ожидался код 401, получен {response.status_code}"
        assert response.json()["message"] == "email or password are incorrect", "Неверное сообщение об ошибке"